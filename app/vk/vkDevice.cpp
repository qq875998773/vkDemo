...
namespace Engine
{
    ...
    {
        buffer->device = GetLogicalDrives();
        ...
        vkCreateBuffer(buffer->device, &bufferCreateInfo, nullptr, &buffer->buffer);

        // Create the memory backing up the buffer handle
        VkMemoryRequirements memReqs;
        VkMemoryAllocateInfo memAlloc = {};
        memAlloc.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        vkGetBufferMemoryRequirements(buffer->device, buffer->buffer, &memReqs);
        memAlloc.allocationSize = memReqs.size;
        // Find a memory type index that fits the properties of the buffer
        memAlloc.memoryTypeIndex = Engine::VulkanDevice::findMemoryTypeIndex(memReqs.memoryTypeBits, memoryPropertyFlags);
        vkAllocateMemory(buffer->device, &memAlloc, nullptr, &buffer->memory);
