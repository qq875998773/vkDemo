...
namespace VkAPI 
{
	UIOverlay::UIOverlay(VkAPI::UIOverlayCreateInfo createInfo)
	{
	...
	void UIOverlay::prepareResources()
	{
		...
		VkDeviceSize uploadSize = (uint64_t)texWidth*texHeight * 4 * sizeof(char);

		// Create target image for copy
		VkImageCreateInfo imageInfo = VkAPI::initializers::imageCreateInfo();
		...
		VkMemoryAllocateInfo memAllocInfo = VkAPI::initializers::memoryAllocateInfo();
		...
		// Image view
		VkImageViewCreateInfo viewInfo = VkAPI::initializers::imageViewCreateInfo();
		...
		VkSamplerCreateInfo samplerInfo = VkAPI::initializers::samplerCreateInfo();
		...
		VkCommandBufferAllocateInfo cmdBufAllocateInfo =
			VkAPI::initializers::commandBufferAllocateInfo(commandPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY, static_cast<uint32_t>(cmdBuffers.size()));
		VK_CHECK_RESULT(vkAllocateCommandBuffers(createInfo.device->logicalDevice, &cmdBufAllocateInfo, cmdBuffers.data()));

		// Descriptor pool
		std::vector<VkDescriptorPoolSize> poolSizes = {
			VkAPI::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1)
		};
		VkDescriptorPoolCreateInfo descriptorPoolInfo = VkAPI::initializers::descriptorPoolCreateInfo(poolSizes, 2);
		...
		std::vector<VkDescriptorSetLayoutBinding> setLayoutBindings = {
			VkAPI::initializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT, 0),
		};
		VkDescriptorSetLayoutCreateInfo descriptorLayout = VkAPI::initializers::descriptorSetLayoutCreateInfo(setLayoutBindings);
		VK_CHECK_RESULT(vkCreateDescriptorSetLayout(createInfo.device->logicalDevice, &descriptorLayout, nullptr, &descriptorSetLayout));

		// Descriptor set
		VkDescriptorSetAllocateInfo allocInfo = VkAPI::initializers::descriptorSetAllocateInfo(descriptorPool, &descriptorSetLayout, 1);
		VK_CHECK_RESULT(vkAllocateDescriptorSets(createInfo.device->logicalDevice, &allocInfo, &descriptorSet));
		VkDescriptorImageInfo fontDescriptor = VkAPI::initializers::descriptorImageInfo(
			sampler,
			fontView,
			VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
		);
		std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
			VkAPI::initializers::writeDescriptorSet(descriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 0, &fontDescriptor)
		};
		...
		VkPushConstantRange pushConstantRange = VkAPI::initializers::pushConstantRange(VK_SHADER_STAGE_VERTEX_BIT, sizeof(PushConstBlock), 0);
		VkPipelineLayoutCreateInfo pipelineLayoutCreateInfo = VkAPI::initializers::pipelineLayoutCreateInfo(&descriptorSetLayout, 1);
		...
		VkFenceCreateInfo fenceCreateInfo = VkAPI::initializers::fenceCreateInfo();
	...
	void UIOverlay::preparePipeline()
	{
		...
		VkPipelineInputAssemblyStateCreateInfo inputAssemblyState =
			VkAPI::initializers::pipelineInputAssemblyStateCreateInfo(VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, 0, VK_FALSE);

		VkPipelineRasterizationStateCreateInfo rasterizationState =
			VkAPI::initializers::pipelineRasterizationStateCreateInfo(VK_POLYGON_MODE_FILL, VK_CULL_MODE_NONE, VK_FRONT_FACE_COUNTER_CLOCKWISE);
		...
		VkPipelineColorBlendStateCreateInfo colorBlendState =
			VkAPI::initializers::pipelineColorBlendStateCreateInfo(static_cast<uint32_t>(blendStates.size()), blendStates.data());

		VkPipelineDepthStencilStateCreateInfo depthStencilState =
			VkAPI::initializers::pipelineDepthStencilStateCreateInfo(VK_FALSE, VK_FALSE, VK_COMPARE_OP_LESS_OR_EQUAL);

		VkPipelineViewportStateCreateInfo viewportState =
			VkAPI::initializers::pipelineViewportStateCreateInfo(1, 1, 0);

		VkPipelineMultisampleStateCreateInfo multisampleState =
			VkAPI::initializers::pipelineMultisampleStateCreateInfo(createInfo.rasterizationSamples);
		...
		VkPipelineDynamicStateCreateInfo dynamicState =
			VkAPI::initializers::pipelineDynamicStateCreateInfo(dynamicStateEnables);

		VkGraphicsPipelineCreateInfo pipelineCreateInfo = VkAPI::initializers::pipelineCreateInfo(pipelineLayout, renderPass);
		...
		std::vector<VkVertexInputBindingDescription> vertexInputBindings = {
			VkAPI::initializers::vertexInputBindingDescription(0, sizeof(ImDrawVert), VK_VERTEX_INPUT_RATE_VERTEX),
		};
		std::vector<VkVertexInputAttributeDescription> vertexInputAttributes = {
			VkAPI::initializers::vertexInputAttributeDescription(0, 0, VK_FORMAT_R32G32_SFLOAT, offsetof(ImDrawVert, pos)),	// Location 0: Position
			VkAPI::initializers::vertexInputAttributeDescription(0, 1, VK_FORMAT_R32G32_SFLOAT, offsetof(ImDrawVert, uv)),	// Location 1: UV
			VkAPI::initializers::vertexInputAttributeDescription(0, 2, VK_FORMAT_R8G8B8A8_UNORM, offsetof(ImDrawVert, col)),	// Location 0: Color
		};
		VkPipelineVertexInputStateCreateInfo vertexInputState = VkAPI::initializers::pipelineVertexInputStateCreateInfo();
	...
	void UIOverlay::updateCommandBuffers()
	{
		...
		VkRenderPassBeginInfo renderPassBeginInfo = VkAPI::initializers::renderPassBeginInfo();
		...
		for (size_t i = 0; i < cmdBuffers.size(); ++i) {
			...
			if (VkAPI::debugmarker::active) {
				VkAPI::debugmarker::beginRegion(cmdBuffers[i], "UI overlay", glm::vec4(1.0f, 0.94f, 0.3f, 1.0f));
			}
			...
			VkViewport viewport = VkAPI::initializers::viewport(ImGui::GetIO().DisplaySize.x, ImGui::GetIO().DisplaySize.y, 0.0f, 1.0f);
			vkCmdSetViewport(cmdBuffers[i], 0, 1, &viewport);

			VkRect2D scissor = VkAPI::initializers::rect2D((int32_t)ImGui::GetIO().DisplaySize.x, (int32_t)ImGui::GetIO().DisplaySize.y, 0, 0);
			...
			if (VkAPI::debugmarker::active) {
				VkAPI::debugmarker::endRegion(cmdBuffers[i]);
			}
