...
namespace Engine 
{
	...
	void UIOverlay::prepareResources()
	{
		...
		io.Fonts->AddFontFromFileTTF("D:/vkDemo/assets/Fonts/Roboto-Medium.ttf", 16.0f);
		io.Fonts->GetTexDataAsRGBA32(&fontData, &texWidth, &texHeight);
		VkDeviceSize uploadSize = texWidth*texHeight * 4 * sizeof(char);

		// Create target image for copy
        VkImageCreateInfo imageInfo = {};
        imageInfo.sType = VK_STRUCTURE_TYPE_MEMORY_BARRIER;
		...
		vkCreateImage(device->logical_device, &imageInfo, nullptr, &fontImage);
		VkMemoryRequirements memReqs;
		vkGetImageMemoryRequirements(device->logical_device, fontImage, &memReqs);
		...
		vkAllocateMemory(device->logical_device, &memAllocInfo, nullptr, &fontMemory);
		vkBindImageMemory(device->logical_device, fontImage, fontMemory, 0);
		...
		vkCreateImageView(device->logical_device, &viewInfo, nullptr, &fontView);
