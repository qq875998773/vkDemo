...
namespace VkAPI
{
	...
	struct Model {
		...
		VkAPI::Buffer vertices;
		VkAPI::Buffer indices;
		...
		*/
		bool loadFromFile(const std::string& filename, VkAPI::VertexLayout layout, VkAPI::ModelCreateInfo *createInfo, VkAPI::VulkanDevice *device, VkQueue copyQueue, const int flags = defaultFlags)
		{
			...
			if (pScene)
			{
				...
				VkAPI::Buffer vertexStaging, indexStaging;
		...
		* @param (Optional) flags ASSIMP model loading flags
		*/
		bool loadFromFile(const std::string& filename, VkAPI::VertexLayout layout, float scale, VkAPI::VulkanDevice *device, VkQueue copyQueue, const int flags = defaultFlags)
		{
			VkAPI::ModelCreateInfo modelCreateInfo(scale, 1.0f, 0.0f);
